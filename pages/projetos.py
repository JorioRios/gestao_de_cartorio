...
with st.container(border=True):
    ...
    if st.checkbox('**Ver relatório em tempo real dos protocolos**'):

        df = pd.read_excel('dados/previsao.xlsx')
        ...
        with st.container(border=True):
            selected_event = st_echarts(
                options=options, events=events, height="400px", key=f"render_grouped_bar_events{key_suffix}"
            )
...
if st.checkbox('Relatório de senhas presenciais'):
    pass